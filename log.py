...
STDOUT_LOG_FMT = "%(log_color)s[%(asctime)s] [%(levelname)s][pid:%(process)s][%(threadName)s:%(thread)d] [%(filename)s:%(lineno)d] [%(module)s:%(funcName)s] %(message)s"
STDOUT_DATE_FMT = "%Y-%m-%d %H:%M:%S"
FILE_LOG_FMT = "[%(asctime)s] [%(levelname)s][pid:%(process)s] [%(threadName)s:%(thread)d] [%(filename)s:%(lineno)d][%(module)s:%(funcName)s] %(message)s"
...
# logger = get_logger(log_filename='./logs/default.log')
# logger.debug("Hello!")
# logger.info("233")
